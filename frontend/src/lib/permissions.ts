...
export const MODULE_LABELS: Record<Module, string> = { dashboard:"داشبورد", proforma:"پیش‌فاکتورها", invoice:"فاکتورها", goods_issue:"حواله خروج", products:"کالاها و قیمت‌ها", customers:"مشتریان", inventory:"انبار", payments:"دریافت‌ها", reports:"گزارشات", company:"اطلاعات شرکت" };
export function moduleForPath(path: string): Module | "users" | undefined {
  ...
  if (path.startsWith("/exchange")) return "reports";
  if (path.startsWith("/returns")) return "invoice";
...
export const FIRST_PATHS = ["/", "/documents/proforma", "/documents/invoice", "/documents/goods-issue", "/products", "/customers", "/inventory", "/payments", "/reports", "/settings/company"];
